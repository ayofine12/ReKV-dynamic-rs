...
MIN_RS=${MIN_RS:-16}
MAX_RS=${MAX_RS:-64}
BETAS=${BETAS:-"0.5 0.6 0.7 0.8 0.9 0.95"}
...
RUN_NAME=${RUN_NAME:-relative_mass_min${MIN_RS}_max${MAX_RS}}
...
echo "[lvbench-relative-reuse] betas=${BETAS}, min_rs=${MIN_RS}, max_rs=${MAX_RS}, num_chunks=${NUM_CHUNKS}, gpus=${GPUS}, n_local=${RUN_N_LOCAL}, save_root=${save_root}"
mkdir -p "${save_root}"

pids=()
for chunk_idx in $(seq 0 $((NUM_CHUNKS - 1))); do
  gpu="${gpu_list[chunk_idx]}"
  echo "[lvbench-relative-reuse] launch chunk ${chunk_idx}/${NUM_CHUNKS} on GPU ${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" video_qa/rekv_offline_vqa.py \
    --model "${MODEL}" \
    --sample_fps "${SAMPLE_FPS}" \
    --n_local "${RUN_N_LOCAL}" \
    --retrieve_size "${MAX_RS}" \
    --relative_retrieve_betas "${BETAS}" \
    --relative_retrieve_min_size "${MIN_RS}" \
    --relative_retrieve_max_size "${MAX_RS}" \
    --relative_retrieve_normalize "${NORMALIZE}" \
    --save_dir "${save_root}" \
    --anno_path "${ANNO_PATH}" \
    --debug "${DEBUG}" \
...
if (( failed )); then
  echo "[lvbench-relative-reuse] at least one chunk failed" >&2
  exit 1
fi

for beta in ${BETAS}; do
  beta_label=${beta//./p}
  run_dir="${save_root}/beta${beta_label}-${SAMPLE_FPS}"
