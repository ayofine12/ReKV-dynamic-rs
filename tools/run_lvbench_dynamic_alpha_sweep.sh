...
for alpha in ${ALPHAS}; do
  alpha_label=${alpha//./p}
  save_dir="${OUTPUT_ROOT}/${MODEL}/lvbench/${RUN_NAME}/alpha${alpha_label}-${SAMPLE_FPS}"
  echo "[lvbench-dynamic] alpha=${alpha}, min_rs=${MIN_RS}, max_rs=${MAX_RS}, gpu=${GPU}, n_local=${RUN_N_LOCAL}, save_dir=${save_dir}"
  mkdir -p "${save_dir}"
...
"${PYTHON}" tools/summarize_lvbench_dynamic_alpha.py \
  --base_dir "${OUTPUT_ROOT}/${MODEL}/lvbench/${RUN_NAME}" \
  --sample_fps "${SAMPLE_FPS}" \
  --max_rs "${MAX_RS}"
