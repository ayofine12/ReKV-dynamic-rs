...
RETRIEVE_SIZES=${RETRIEVE_SIZES:-"16 64"}
...
# Skip videos longer than 1 hour by default. Set MAX_DURATION= to include all.
...
echo "[mlvu-fixed-reuse] retrieve_sizes=${RETRIEVE_SIZES}, max_rs=${MAX_RS}, num_chunks=${NUM_CHUNKS}, gpus=${GPUS}, n_local=${RUN_N_LOCAL}, save_root=${save_root}"
mkdir -p "${save_root}"

pids=()
for chunk_idx in $(seq 0 $((NUM_CHUNKS - 1))); do
  gpu="${gpu_list[chunk_idx]}"
  echo "[mlvu-fixed-reuse] launch chunk ${chunk_idx}/${NUM_CHUNKS} on GPU ${gpu}"
...
if (( failed )); then
  echo "[mlvu-fixed-reuse] at least one chunk failed" >&2
  exit 1
fi
