...
if [[ -z "${RUN_NAME:-}" ]]; then
  RUN_NAME="relative_mass_min${MIN_RS}_max${MAX_RS}"
...
echo "[mlvu-relative-reuse] betas=${BETAS}, min_rs=${MIN_RS}, max_rs=${MAX_RS}, num_chunks=${NUM_CHUNKS}, gpus=${GPUS}, n_local=${RUN_N_LOCAL}, save_root=${save_root}"
mkdir -p "${save_root}"

pids=()
for chunk_idx in $(seq 0 $((NUM_CHUNKS - 1))); do
  gpu="${gpu_list[chunk_idx]}"
  echo "[mlvu-relative-reuse] launch chunk ${chunk_idx}/${NUM_CHUNKS} on GPU ${gpu}"
...
if (( failed )); then
  echo "[mlvu-relative-reuse] at least one chunk failed" >&2
  exit 1
fi
